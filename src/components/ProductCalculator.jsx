...
function ResultCard({ label, value, unit, note }) {
  return (
    <div className="bg-lawn-50 border border-lawn-200 rounded-xl p-4 text-center">
      <p className="text-xs font-semibold text-lawn-700 uppercase tracking-wider mb-1">{label}</p>
      <p className="text-3xl font-extrabold text-lawn-800">{value}<span className="text-lg ml-1 font-semibold text-lawn-600">{unit}</span></p>
      {note && <p className="text-xs text-lawn-600 mt-1">{note}</p>}
    </div>
  )
}
  ...
  return (
    <div className="card">
      <h2 className="section-title">
        <Calculator className="w-5 h-5 text-earth-600" />
        Product Calculator
      </h2>
      ...
      <div className="mb-5">
        ...
        <div className="flex gap-2 items-center">
          ...
          />
          <span className="text-stone-500 font-medium">sq ft</span>
          {sqft > 0 && (
            <span className="text-stone-400 text-sm">≈ {(sqft / 43560).toFixed(2)} acres</span>
          )}
        </div>
        {!sqft && (
          <p className="text-xs text-stone-400 mt-1">Tip: multiply length × width to estimate, or use a mapping tool like Google Maps.</p>
        )}
      </div>
        ...
        {TABS.map(({ id, label, Icon }) => (
          <button
            ...
            className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 py-2 px-2 rounded-lg text-xs sm:text-sm font-semibold transition-colors duration-150
              ${activeTab === id ? 'bg-white shadow text-lawn-800' : 'text-stone-500 hover:text-stone-700'}`}
          >
      ...
      {activeTab === 'seed' && (
        <div className="space-y-4">
          ...
          {seedResult && sqft > 0 ? (
            ...
            <p className="text-stone-400 text-sm text-center py-3">Enter lawn square footage above to calculate.</p>
          )}

          <div className="bg-amber-50 rounded-xl p-3 text-amber-900 text-xs">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-stone-400 mt-1">E.g. "30" from a 30-0-4 bag label</p>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-stone-500 text-sm">lb N</span>
              </div>
              <p className="text-xs text-stone-400 mt-1">Standard is 1 lb N / 1,000 sqft</p>
            </div>
          </div>
          ...
          {fertResult && sqft > 0 ? (
            <div className="grid grid-cols-3 gap-3">
              ...
              <ResultCard label="Per 1,000 sqft" value={fertResult.per1000sqft} unit="lbs"
                note="Spreader calibration" />
              <ResultCard label="Actual N Delivered" value={fertResult.totalLbsN} unit="lbs N" />
            </div>
          ) : (
            <p className="text-stone-400 text-sm text-center py-3">Enter your lawn size and N% to calculate.</p>
          )}
            ...
            <div className="flex gap-3">
              {[1, 2, 3, 4].map(d => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg font-semibold text-sm border-2 transition-colors
                    ${mulchDepth === d ? 'bg-earth-600 border-earth-600 text-white' : 'border-stone-200 text-stone-600 hover:border-earth-400'}`}
                >
                  {d}"
                </button>
              ))}
            </div>
            <p className="text-xs text-stone-400 mt-1">Standard recommendation: 2–3 inches. Never pile against tree trunks.</p>
          </div>

          {mulchResult && sqft > 0 ? (
            <div className="grid grid-cols-3 gap-3">
              <ResultCard label="Cubic Yards" value={mulchResult.cubicYards} unit="cu yd"
                note="For bulk delivery" />
              <ResultCard label="Cubic Feet" value={mulchResult.cubicFeet} unit="cu ft" />
              <ResultCard label="Bags Needed" value={mulchResult.bagsNeeded} unit="bags"
                note="Standard 2-cu-ft bags" />
            </div>
          ) : (
            <p className="text-stone-400 text-sm text-center py-3">Enter bed/area square footage above to calculate.</p>
          )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-stone-500 text-sm">lbs / 1k sqft</span>
            </div>
            <p className="text-xs text-stone-400 mt-1">Typical granular range: 2.5–3.25 lbs/1,000 sqft. Always check your product label.</p>
          </div>

          {preResult && sqft > 0 ? (
            <div className="grid grid-cols-2 gap-3">
              <ResultCard label="Product Needed" value={preResult.lbsNeeded} unit="lbs" />
              <ResultCard label="Application Rate" value={preResult.ratePerThousand} unit="lbs/1k sqft"
                note="Per product label" />
            </div>
          ) : (
            <p className="text-stone-400 text-sm text-center py-3">Enter your lawn size above to calculate.</p>
          )}

          <div className="bg-sky-50 rounded-xl p-3 text-sky-900 text-xs">
            <strong>Timing:</strong> Apply when soil temperature at 2-inch depth reaches 50°F in early spring—use a soil thermometer for accuracy. A second application 6–8 weeks later extends the barrier.
          </div>
        </div>
      )}
    </div>
